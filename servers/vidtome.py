...
def test_video_exists(page_url):
    ...
    if "Not Found" in data or "File Does not Exist" in data:
        return False, config.getLocalizedString(70292) % 'Vidto.me'
    return True, ""
...
def get_videoUrl(page_url, premium=False, user="", password="", video_password=""):
    ...
    for p in packed:
        ...
        videoUrls.extend(support.get_jwplayer_mediaUrl(data, 'vidtome'))
        # media_url = scrapertools.find_single_match(data, r"source:\\'([^\\']+)")
        # if media_url:
        #     videoUrls.append([media_url.split('.')[-1] + ' [Vidto.me]', media_url])
    return videoUrls
