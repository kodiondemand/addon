...
def new_search(item):
    ...
    temp_search_file = config.getTempFile('temp-search')
    ...
    if item.search_text:
        ...
        searched_text = platformtools.dialogInput(default=last_search, heading='')

    save_search(searched_text)
    if not searched_text:
        return

    channeltools.setChannelSetting('Last_searched', searched_text, 'search')
...
def channel_search(item):
    ...
    cnt = 0
    progress = platformtools.dialogProgress(config.getLocalizedString(30993) % item.title, config.getLocalizedString(70744) % len(channel_list) + '\n' + ', '.join(searching_titles))
    config.setSetting('tmdb_active', False)
    search_action_list = []
    module_dict = {}
    for ch in channel_list:
        try:
            module = platformtools.channelImport(ch)
    ...
    with futures.ThreadPoolExecutor(max_workers=set_workers()) as executor:
        ...
        for res in futures.as_completed(c_results):
            ...
            for it in search_action_list:
                ...
                progress.update(old_div(((total_search_actions - len(search_action_list)) * 100), total_search_actions), config.getLocalizedString(70744) % str(len(channel_list) - cnt) + '\n' + ', '.join(searching_titles))

    progress.close()

    cnt = 0
    progress = platformtools.dialogProgress(config.getLocalizedString(30993) % item.title, config.getLocalizedString(60295) + '\n' + config.getLocalizedString(60293))
    ...
    for key, value in ch_list.items():
        ...
        grouped = list()
        cnt += 1
        progress.update(old_div((cnt * 100), len(ch_list)), config.getLocalizedString(60295))
        ...
        for it in value:
            ...
            if mode == 'all' or (it.contentType and mode == it.contentType):
                if config.getSetting('result_mode') != 0:
                    if config.getLocalizedString(30992) not in it.title:
        ...
        # to_temp[key] = grouped
        if config.getSetting('result_mode') == 0:
            if not config.getSetting('unify'):
                title = typo(ch_name,'bold') + typo(str(len(grouped)), '_ [] color kod bold')
            else:
                title = typo('%s %s' % (len(grouped), config.getLocalizedString(70695)), 'bold')
    ...
    if config.getSetting('result_mode') == 1 and mode == 'movie':
        progress = platformtools.dialogProgress(config.getLocalizedString(30993) % item.title, config.getLocalizedString(60683))
        valid_servers = []
        with futures.ThreadPoolExecutor(max_workers=set_workers()) as executor:
    ...
    results_statistic = config.getLocalizedString(59972) % (item.title, time.time() - start)
    if mode == 'all':
        results.insert(0, Item(title=typo(results_statistic, 'color kod bold'), thumbnail=thumb('search')))
    else:
        if not valid:
            valid.append(Item(title=config.getLocalizedString(60347), thumbnail=thumb('nofolder')))

        valid.insert(0, Item(title=typo(results_statistic, 'color kod bold'), thumbnail=thumb('search')))
        results.insert(0, Item(title=typo(config.getLocalizedString(30025), 'color kod bold'), thumbnail=thumb('search')))
...
def get_channel_results(item, module_dict, search_action):
    ...
    try:
        ...
        if len(results) == 1:
            if not results[0].action or config.getLocalizedString(70006).lower() in results[0].title.lower():
                results.clear()
        if item.mode != 'all':
            for elem in results:
        ...
        #
        #     if len(results) == 1:
        #         if not results[0].action or config.getLocalizedString(30992).lower() in results[0].title.lower():
        #             return [ch, []]
        #
...
def settings(item):
    return platformtools.showChannelSettings(caption=config.getLocalizedString(59993))
...
def years_menu(item):
    ...
    thumb = support.thumb('movie_year')
    ...
    itemlist.reverse()
    itemlist.append(Item(channel=item.channel, title=typo(config.getLocalizedString(70745),'color kod bold'), url='',
...
def year_cus(item):
    ...
    heading = config.getLocalizedString(70042)
    year = platformtools.dialogNumeric(0, heading, default="")
...
def actor_list(item):
    ...
    if len(results) >= config.getSetting('pagination', default=20):
        next_ = item.page + 1
...
def discover_list(item):
        ...
        if not item.cast_ or (item.cast_ and (int(year) <= int(datetime.datetime.today().year))):
            if config.getSetting('new_search'):
                new_item = Item(channel='globalsearch', title=title, infoLabels=elem,
                                action='new_search', text=title,
                                thumbnail=thumbnail, fanart=fanart,
                                context='', mode = 'search/' + mode, contentType=mode,
    ...
    elif config.getSetting('pagination', default=20):
        if item.discovery:
...
def get_from_temp(item):
    ...
    if nextp < nTotal:
        results.append(Item(channel='search', title=typo(config.getLocalizedString(30992), 'bold color kod'),
