...
def set_extra_values(item, json, path):
    ...
    for key in json:
        if key == 'quality':
            ...
            ret.thumb = json[key] if ':/' in json[key] else filetools.join(path, json[key]) if '/' in json[key] else support.get_thumb(json[key])
        elif key == 'fanart':
...
def relative(key, json, path):
    ...
    if key in json:
        if key in ['thumbnail', 'poster']:
            ret = json[key] if ':/' in json[key] else filetools.join(path, json[key]) if '/' in json[key] else support.get_thumb(json[key]) if json[key] else ''
        else:
